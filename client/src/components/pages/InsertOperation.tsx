  ...
  function handleSubmit(e: any) {
    ...
    if (nameOperation && getValue && preference && qttNote && qttPackage) {
      ...
      if (rest !== 0) {
        ...
        axios
          .post(
            "http://localhost:3001/createOperation",
            {
                name_operation: nameOperation,
                value: getValue,
                preference: preference,
                qtt_notes: qttNote,
                qtt_packages: qttPackage,
            },
            {
              headers: {
  ...
  return (
    <div className="section-insertoperation">
      ...
      <form className="insertoperation-form" onSubmit={handleSubmit}>
        <label className="form-label form-label-name">
          ...
          <input
            ...
            type="text"
            required
            onChange={handleNameOperationText}
            value={nameOperation}
          />
        </label>
        <label className="form-label form-label-value">
          <span>Valor: R$</span>
          <input
            className="label-input label-input-value"
            type="number"
        ...
        <label className="form-label form-label-quantities">
          ...
          <input
            className="label-input label-input-quantities"
            type="text"
            disabled
            value={qttNote}
          />
        ...
        <label className="form-label form-label-packages">
          ...
          <input
            className="label-input label-input-package"
            type="text"
            disabled
            value={qttPackage}
          />
      ...
      <div className="insertoperation-observations">
        ...
        <div className="observation-default">
          - O valor tem que ser de 100 em 100.{" "}
          <span>Ex: R$1000, R$1100, R$1200..</span>
        </div>
        <div className="observation-default">
          - Terá somente 1 tipo de preferência de notas.{" "}
          <span>(R$10 / R$50 / R$100)</span>
        </div>
        <div className="observation-default">
          - Cada Pacote terá o máximo de 50 notas. Se houver mais, será dividido
          em subpacotes
        </div>
      </div>
    </div>
